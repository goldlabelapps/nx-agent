...
This repo currently uses `agents/leida-chat/`.
...
To align this project with Eve defaults while keeping Leida docs and product structure:
...
5. Decide whether to keep `agents/leida-chat/` or migrate to `agent/` for convention alignment.

## Decision Log

Implemented: adopted official Eve runtime defaults and migrated to `agent/` while preserving Leida-specific docs and tools.
